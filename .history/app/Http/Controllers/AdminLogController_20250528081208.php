...
class AdminLogController extends Controller
{
    ...
    /**
     * Group logs by action and target, aggregating server details.
     * 
     * @param array $logs Array of log objects
     * @return array Grouped logs
     */
    private function groupLogsByActionAndTarget($logs)
    {
        $grouped = [];
        ...
        foreach ($logs as $log) {
            ...
            $key = "{$log->action}_{$log->target_id || 'null'}_{$log->admin_id}_{$dateKey}";
            ...
            if ($log->details && is_object($log->details) && isset($log->details->server_id)) {
                $serverId = $log->details->server_id;
                $serverName = $log->details->server_name ?? "Server #{$serverId}";
                
                if (!in_array($serverId, array_column($grouped[$key]['servers'], 'id'))) {
                    $grouped[$key]['servers'][] = [
                        'id' => $serverId,
                        'name' => $serverName
                    ];
                    ...
                    $grouped[$key]['multiple_details'][] = $log->details;
                }
            } else if ($log->details) {
                $grouped[$key]['multiple_details'][] = $log->details;
            }
