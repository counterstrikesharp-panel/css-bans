...
class AdminLogController extends Controller
{
    ...
    public function show($id)
    {
        ...
        // Parse the details JSON for each server
        $serverDetails = [];
        if ($log->details) {
            // If details is a json string, decode it first
            $details = is_string($log->details) ? json_decode($log->details) : $log->details;
            
            // Check if it's a single server action or multiple servers
            if (is_array($details)) {
                $serverDetails = $details;
            } else {
                $serverDetails = [$details]; // Convert to array for consistent processing
            }
        }
        
        return view('admin.logs.show', compact('log', 'serverDetails'));
    }
}
