            ...
            $(document).ready(function() {
                ...
                $(document).on('click', '.view-details', function(e) {
                    ...
                    try {
                        ...
                        // Clean up HTML from details if it exists
                        if (typeof detailsData === 'string' && detailsData.includes('<button')) {
                            // Extract the JSON part from button data-details attribute
                            const match = detailsData.match(/data-details='(.+?)'/);
                            if (match && match[1]) {
                                try {
                                    detailsData = JSON.parse(match[1]);
                                } catch (htmlParseError) {
                                    console.error("Error parsing HTML details:", htmlParseError);
                                }
                            } else {
                                // If we can't extract JSON, show the raw data without HTML
                                detailsData = detailsData.replace(/<[^>]*>/g, '');
                            }
                        }
                        
                        // Format the details for display
                        const formattedDetails = JSON.stringify(detailsData, null, 2);
                        $('#details-content').text(formattedDetails);
                    } catch (error) {
                        // Fallback if any error occurs
                        console.error("Error processing details:", error);
                        $('#details-content').text("Could not parse details data: " + JSON.stringify(detailsData));
                    }
