        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                $(document).on('click', '.view-details', function(e) {
                    ...
                    try {
                        // Get the raw details data directly from the button
                        ...
                        if (detailsData === null || detailsData === undefined) {
                            $('#details-content').text("No details available");
                        } else if (typeof detailsData === 'object') {
                            $('#details-content').text(JSON.stringify(detailsData, null, 2));
                        } else {
                            $('#details-content').text(detailsData);
                        }
                    } catch (error) {
                        console.error("Error displaying details:", error);
                        $('#details-content').text("Error displaying details data");
                    }
            ...
            // Update the row creation function to properly handle the details data
            function createTableRow(log) {
                ...
                let detailsAttr = '{}';
                if (log.details) {
                    // Make sure it's properly JSON encoded for the HTML attribute
                    if (typeof log.details === 'object') {
                        detailsAttr = JSON.stringify(log.details).replace(/"/g, '&quot;');
                    } else if (typeof log.details === 'string') {
                        // Check if already JSON
                        try {
                            // Try to parse to validate it's proper JSON
                            JSON.parse(log.details);
                            // If valid, escape quotes
                            detailsAttr = log.details.replace(/"/g, '&quot;');
                        } catch (e) {
                            // If not valid JSON, stringify the string itself
                            detailsAttr = JSON.stringify(log.details).replace(/"/g, '&quot;');
                        }
                    }
                }
