    ...
    <x-slot:footerFiles>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                // Replace the click handler for view-details with this improved version
                $(document).on('click', '.view-details', function(e) {
                    ...
                    if (Array.isArray(detailsData)) {
                        ...
                        $('#serverCountBadge').html(`<span class="badge badge-info">${detailsData.length} servers</span>`);
                        
                        let content = '<div class="p-2">';
                        detailsData.forEach((item, index) => {
                            let serverName = "Unknown Server";
                            if (item && typeof item === 'object') {
                                serverName = item.server_name || `Server #${item.server_id || index + 1}`;
                            }
                            
                            content += `<div class="server-detail mb-3 p-3" style="background: #1e2a4a; border-radius: 6px;">
                                <h6 class="mb-2 text-white">${serverName}</h6>
                                <pre style="background: #0e1726; color: #bfc9d4; padding: 10px; border-radius: 4px; max-height: 200px; overflow-y: auto;">${JSON.stringify(item, null, 2)}</pre>
                            </div>`;
                        });
                        content += '</div>';
