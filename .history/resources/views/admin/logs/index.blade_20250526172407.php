                ...
                function loadLogs(reset = false) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            // Update stats
                            $('#total-logs-count').text(response.stats.total);
                            $('#ban-count').text(response.stats.bans);
                            $('#unban-count').text(response.stats.unbans);
                            $('#other-actions-count').text(response.stats.others);
                            ...
                            response.data.forEach(function(log, index) {
                                ...
                                const actionClass = log.action.replace(/_/g, '-');
                                
                                // Handle target text
                                let targetText = '';
                                if (log.target_name) {
                                    targetText = `<div class="timeline-target">Target: ${log.target_name}</div>`;
                                }
                                ...
                                $('#timeline').append(`
                                    <div class="timeline-item ${position}">
                                        <div class="timeline-content">
                                            ...
                                            <div class="timeline-action ${actionClass}">${log.action.replace(/_/g, ' ').toUpperCase()}</div>
                                            <div>${log.description}</div>
                                            ${targetText}
                                            <div class="mt-2">
                                                <small>By: ${log.admin_name}</small>
                                            </div>
                                            <button class="btn btn-sm btn-info mt-2 log-details-btn" data-id="${log.id}">Details</button>
                                        </div>
                                    </div>
                                `);
                            });
                            ...
                            if (hasMorePages) {
                                $('#load-more').html('Load More').prop('disabled', false).show();
                            } else {
                                $('#load-more').hide();
                            }
                        },
                        error: function() {
                            isLoading = false;
                            $('#timeline').html('<div class="alert alert-danger">Error loading logs. Please try again.</div>');
                            $('#load-more').hide();
                        }
                    });
                }
