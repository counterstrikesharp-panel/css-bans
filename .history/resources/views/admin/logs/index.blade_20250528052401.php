    ...
    <x-slot:footerFiles>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                // Update the click handler for the view details button with fixed error handling
                $(document).on('click', '.view-details', function(e) {
                    e.preventDefault();
                    
                    try {
                        // Get the raw details data and handle weird JSON formats
                        let rawDetails = $(this).attr('data-details');
                        let detailsData;
                        
                        // Clean up the HTML and script tags if present
                        if (typeof rawDetails === 'string' && (rawDetails.includes('<button') || rawDetails.includes('View Details'))) {
                            // Try to extract meaningful data using regex
                            const jsonMatches = rawDetails.match(/data-details=['"]([^'"]+)['"]/i);
                            if (jsonMatches && jsonMatches[1]) {
                                try {
                                    detailsData = JSON.parse(jsonMatches[1].replace(/&quot;/g, '"'));
                                } catch (err) {
                                    detailsData = { error: "Could not parse embedded details" };
                                }
                            } else {
                                detailsData = { error: "Could not extract details from HTML" };
                            }
                        } else {
                            // Try normal JSON parsing with HTML entity decoding
                            try {
                                // Replace HTML entities with actual characters
                                rawDetails = rawDetails.replace(/&quot;/g, '"')
                                                      .replace(/&#039;/g, "'")
                                                      .replace(/&lt;/g, '<')
                                                      .replace(/&gt;/g, '>');
                                detailsData = JSON.parse(rawDetails);
                            } catch (parseError) {
                                console.error("JSON parse error:", parseError);
                                detailsData = { rawData: rawDetails };
                            }
                        }
                        ...
                        if (Array.isArray(detailsData)) {
                            // Handle multiple server details with a simple format
                            ...
                            detailsData.forEach((item, index) => {
                                content += `<div class="server-detail mb-3 p-3" style="background: #1e2a4a; border-radius: 6px;">
                                    <h6 class="mb-2 text-white">Server ${index+1}</h6>
