...
<x-base-layout :scrollspy="false">
    ...
    <div class="container">
        ...
        <div class="action-header">
            ...
            <div class="action-meta">
                ...
                <div class="meta-item">
                    ...
                    <span class="meta-value">{{ count($serverDetails) }}</span>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="timeline">
            @foreach($serverDetails as $index => $details)
            <div class="timeline-item action-{{ strtolower(str_replace('_', '', $log->action)) }}">
                ...
                <div class="timeline-content">
                    <div class="server-header">
                        <div class="server-name">
                            ...
                            {{ $details->server_name ?? 'Server #' . ($details->server_id ?? ($index + 1)) }}
                        </div>
                        <div class="server-badge">Server #{{ $index + 1 }}</div>
                    </div>
                    
                    <!-- Target Information -->
                    @if(!empty($details->target_name) || !empty($details->player_name) || !empty($details->target_steam_id) || !empty($details->player_steamid) || !empty($details->target_ip) || !empty($details->player_ip))
                    <div class="detail-section">
                        ...
                        @if(!empty($details->target_name) || !empty($details->player_name))
                        <div class="detail-row">
                            <div class="detail-label">Name:</div>
                            <div class="detail-value">{{ $details->target_name ?? $details->player_name }}</div>
                        </div>
                        @endif
                        
                        @if(!empty($details->target_steam_id) || !empty($details->player_steamid) || !empty($details->steam_id))
                        <div class="detail-row">
                            <div class="detail-label">Steam ID:</div>
                            <div class="detail-value">
                                <a href="https://steamcommunity.com/profiles/{{ $details->target_steam_id ?? $details->player_steamid ?? $details->steam_id }}" target="_blank" class="person-link">
                                    <i class="fab fa-steam"></i>{{ $details->target_steam_id ?? $details->player_steamid ?? $details->steam_id }}
                                </a>
                            </div>
                        </div>
                        @endif
                        
                        @if(!empty($details->target_ip) || !empty($details->player_ip) || !empty($details->ip))
                        <div class="detail-row">
                            <div class="detail-label">IP Address:</div>
                            <div class="detail-value highlight-info">{{ $details->target_ip ?? $details->player_ip ?? $details->ip }}</div>
                        </div>
                        @endif
                    </div>
                    @endif
                    ...
                    <!-- Action Information -->
                    @if(!empty($details->reason) || !empty($details->duration) || !empty($details->ends))
                    <div class="detail-section">
                        <div class="section-title">
                        ...
                        <div class="detail-row">
                            ...
                            <div class="detail-value highlight-warning">{{ $details->reason }}</div>
                        </div>
                        @endif
                        ...
                        @if(!empty($details->ends))
                        <div class="detail-row">
                            <div class="detail-label">Ends:</div>
                            <div class="detail-value">{{ $details->ends }}</div>
                        </div>
                        @endif
                    </div>
                    @endif
                    ...
                    <div class="detail-section">
                        ...
                        @foreach((array)$details as $key => $value)
                            ...
                            <div class="detail-row">
                                <div class="detail-label">{{ ucwords(str_replace('_', ' ', substr($key, 7))) }}:</div>
                    ...
                    foreach((array)$details as $key => $value) {
                        if (strpos($key, 'server_') !== 0 && 
                            ...
                            $key !== 'reason' && $key !== 'duration' && $key !== 'ends') {
